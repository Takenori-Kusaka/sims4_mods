/**
 * Career Auto Tasks - Build Script Unit Tests
 */
...
describe("Career Auto Tasks MOD", () => {
  ...
  describe("Source Files", () => {
    ...
    it("should have trait XML file", () => {
      const traitFile = path.join(SRC_DIR, "tuning", "traits", "Kokor_Trait_CareerAutoTasks.xml");
      expect(fs.existsSync(traitFile)).to.be.true;
    });
    
    it("should have buff XML file", () => {
      const buffFile = path.join(SRC_DIR, "tuning", "buffs", "Kokor_Buff_CareerFocused.xml");
  ...
  describe("XML Validation", () => {
    ...
    it("trait XML should have correct root element", () => {
      ...
      // XmlResourceがコンテンツを持っていることを確認
      expect(resource.content).to.include("<I c=\"Trait\"");
      expect(resource.content).to.include("n=\"Kokor:Trait_CareerAutoTasks\"");
    });
    ...
    it("buff XML should have correct root element", () => {
      ...
      expect(resource.content).to.include("n=\"Kokor:Buff_CareerFocused\"");
    });
  });
  ...
  describe("Built Package", () => {
    ...
    before(function() {
      ...
      const packagePath = path.join(DIST_DIR, "KokorCareerAutoTasks.package");
  ...
  /**
   * Resource Type ID 検証テスト
   * 
   * 今回の問題の根本原因は間違った Type ID でした。
   * このテストスイートはビルド後のパッケージが正しい Type ID を使用しているか検証します。
   */
